...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Bewerte, wie gut ein Kind/Jugendlicher folgenden unfreundlichen Satz umformuliert hat:

ORIGINAL: "${original}"
UMFORMULIERUNG: "${rewrite}"

Gib konstruktives, ermutigendes Feedback (2-4 Sätze). Lobe positive Aspekte und gib konkrete Tipps für Verbesserungen. Verwende eine freundliche, unterstützende Sprache.

Gib das Feedback ausschließlich im folgenden JSON-Format zurück:

{
  "feedback": "Dein Feedback hier..."
}`;

    console.log('Checking rewrite...');
    ...
    console.error('Error in check-rewrite function:', error);
    return new Response(JSON.stringify({ 
      error: 'Error checking rewrite',
